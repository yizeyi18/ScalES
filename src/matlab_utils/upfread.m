...
pp.info.pptype = 'upf';
pp.info.version = mapvalue(UPFattr,'version','');
...
pp.info = upfassignattr(pp.info,attr,meshkey,meshinit);
...
if ~isempty(semilocalstr)
    ...
    for ibeta = 1:upf_nbeta
        ...
        pp.semilocal = upfassignattr(pp.semilocal,attr,semilocalkey,...
                                     semilocalinit,ibeta);
...
if ~isempty(nonlocstr)
    ...
    for ibeta = 1:upf_nbeta
        ...
        pp.nonloc = upfassignattr(pp.nonloc,attr,nonlocbetakey,...
...
% Find the value with the given key in the map.
    function val = mapvalue(m,k,dv)
        m = cell2val(m,containers.Map);
        k = {cell2val(k)};
        if isKey(m,k)
            val = str2type(values(m,k),dv);
        else
            val = dv;
        end
