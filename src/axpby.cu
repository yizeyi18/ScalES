...
namespace dgdft {
  namespace device {
    ...
    template <typename T>
      __global__ void add_vecs_kernel( int nVec, int N, T** vecs, T BETA, T* result ) {

        const int tid = blockIdx.x * blockDim.x + threadIdx.x;
        if( tid < N ) {
          result[ tid ] = BETA * result[ tid ]; 
          for( auto i = 0; i < nVec; ++i )
            result[tid] += vecs[i][tid];
        }
    ...
    template <typename T>
      void add_vecs_device( int nVec, int N, T** vecs, T BETA, T* result ) {
        ...
        auto div = std::div( N, 1024 );
        add_vecs_kernel<T><<< div.quot + !!div.rem, 1024 >>>(
            nVec, N, vecs, BETA, result );
    ...
    template
      void add_vecs_device<double>( int nVec, int N, double** vecs, double BETA, double* result );
