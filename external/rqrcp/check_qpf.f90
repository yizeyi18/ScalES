...
DOUBLE PRECISION FUNCTION CHECK_QPF( M, N, K, A, DESCA, B, DESCB, C, DESCC, TAU, IPIV, WORK, LWORK )

IMPLICIT NONE

INTEGER            M, N, K, INFO, LWORK
INTEGER            DESCA( * ), DESCB( * ), DESCC( * ), IPIV( * )
...
! COMPUTE PRODUCT OF C AND INV(IPIV), THE RESULT WILL BE EQUAL TO INPUT MATRIX A
CALL PDQPPIV( M, N, C, 1, 1, DESCC, IPIV )
CALL PDGEADD( 'N', M, N, MONE, A, 1, 1, DESCA, ONE, C, 1, 1,&
   DESCC )

CNORM = PDLANGE( 'F', M, N, C, 1, 1, DESCC, WORK )

! CHECK_QPF SHOULD BE ZERO
CHECK_QPF = CNORM / ANORM
