              ...
              utilities_FortranInterpreter* util )
{
  ...
  // for(int j = 0; j < n; j++){
    // for(int i = 0; i < n; i++){
      // fprintf(stderr,"a[%4d,%4d] = (%10.2e, %10.2e)\n", i, j, aval[i+j*lda].real, aval[i+j*lda].imag);
    // }
  // }
    // for(int i = 0; i < n; i++){
      // fprintf(stderr,"a[%4d,%4d] = (%10.2e, %10.2e)\n", i, i, aval[i+i*lda].real, aval[i+i*lda].imag);
    // }
...
// 
  ...
  // LLIN: Debuging info for the condition number information.
  // if(0){
    // int info1, ierr;
    // double rcond, anorm = 1.0;  //LLIN: Guess for ANORM
    // double *bnew;
    // int *iwork;
    // bnew = (double*)malloc(sizeof(double)*ldb*n);
    // iwork = (int*)malloc(sizeof(int)*n);
    // for(int i = 0; i < ldb * n; i++){
      // bnew[i] = b[i];
    // }
    // blap_fn.dpotrf(&uplo, &n, &bnew[0], &ldb, &ierr);
    // dpocon_(&uplo, &n, &bnew[0], &ldb, &anorm, &rcond, &work[0],
	    // &iwork[0], &info1);
    // // for(int i = 0; i < n; i++){
      // // fprintf(stderr,"bnew[%5d,%5d] = %10.5e \n",  i,i, bnew[i+i*lda]);
    // // }
    // // fprintf(stderr,"n = %5d,  ldb = %5d \n",  n, ldb);
    // fprintf(stderr,"Condition number of B = %10.5e\n",  1./ rcond);
    // free(bnew);
    // free(iwork);
  // }
                   ...
                   utilities_FortranInterpreter* util )
{
  ...
  // if(0){
    ...
    // double rcond, anorm = 1.0; //LLIN: Guess for ANORM
    // komplex *bnew;
    // bnew = (komplex*)malloc(sizeof(komplex)*ldb*n);
    // for(int i = 0; i < ldb * n; i++){
      // bnew[i].real = b[i].real;
      // bnew[i].imag = b[i].imag;
    // }
    // blap_fn.zpotrf(&uplo, &n, &bnew[0], &ldb, &ierr);
    // zpocon_(&uplo, &n, &bnew[0], &ldb, &anorm, &rcond, &work[0],
	    // &rwork[0], &info1);
    // fprintf(stderr,"Condition number of B = %10.5e\n",  1.0 / rcond);
    // free(bnew);
  // }
