			...
			for(int i1=0; i1<_NElems[1]; i1++)
				for(int i0=0; i0<_NElems[0]; i0++) {
					if(_elemptn.owner(Index3(i0,i1,i2))==myid) {
						...
						cout << "Interpolation again " << endl;

						_vtotvec.lclmap()[Index3(i0,i1,i2)].resize(elem.ntot());
						xlinterp(&(_vtotvec.lclmap()[Index3(i0,i1,i2)][0]), &(buff._vtot)[0], elem, buff); 	    
						
						cout << "Interpolation finish" << endl;
					}
				}

		if(mpirank==0) { fprintf(stderr, "scf scf iter %d vtot interpolation done \n", iter); }
				...
				for(int i1=0; i1<_NElems[1]; i1++)
					for(int i0=0; i0<_NElems[0]; i0++) {
						if(_elemptn.owner(Index3(i0,i1,i2))==myid) {
							...
							cout << "enrich number = " << _enrichTns(i0,i1,i2) << endl;
							iC( eigpw.SolveLOBPCG(buff._vtot, buff._vnls, _enrichTns(i0,i1,i2) + _nbufextra, 
							...
							elembases.resize(_enrichTns(i0,i1,i2));		
							for(int g=0; g<_enrichTns(i0,i1,i2); g++) {
