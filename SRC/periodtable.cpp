	...
	if(1) {
		...
		for(int g=3; g<ptentry.samples().n(); g=g+2) {
			...
			if(typ==3) {
				// f_z3
				{
					double coef = 1.0/4.0*sqrt(7.0/M_PI); // Coefficients for spherical harmonics
					IntNumVec iv(idx.size(), true, &(idx[0]));
					DblNumMat dv(4, idx.size());
					DblNumVec Ylm(4); //LLIN: Spherical harmonics (0) and its derivatives (1-3)
					for(int g=0; g<idx.size(); g++) {
						if(rad[g]>eps) {
							...
							double y2 = yy[g]*yy[g];
							double z2 = zz[g]*zz[g];
							double r3 = pow(rad[g], 3.0);
							double r5 = pow(rad[g], 5.0);
				...
				// f_y(3xx-yy)
				{
					double coef = 1.0/4.0*sqrt(35./(2.*M_PI)); // Coefficients for spherical harmonics
					IntNumVec iv(idx.size(), true, &(idx[0]));
					DblNumMat dv(4, idx.size());
					DblNumVec Ylm(4); //LLIN: Spherical harmonics (0) and its derivatives (1-3)
					for(int g=0; g<idx.size(); g++) {
				...
				// f_z(xx-yy)
				{
					double coef = 1.0/4.0*sqrt(105./(M_PI)); // Coefficients for spherical harmonics
					IntNumVec iv(idx.size(), true, &(idx[0]));
					DblNumMat dv(4, idx.size());
					DblNumVec Ylm(4); //LLIN: Spherical harmonics (0) and its derivatives (1-3)
					for(int g=0; g<idx.size(); g++) {
				...
				// f_xyz
				{
					double coef = 1.0/2.0*sqrt(105./(M_PI)); // Coefficients for spherical harmonics
					IntNumVec iv(idx.size(), true, &(idx[0]));
					DblNumMat dv(4, idx.size());
					DblNumVec Ylm(4); //LLIN: Spherical harmonics (0) and its derivatives (1-3)
					for(int g=0; g<idx.size(); g++) {
				...
				// f_yzz
				{
					double coef = 1.0/4.0*sqrt(21./(2.*M_PI)); // Coefficients for spherical harmonics
					IntNumVec iv(idx.size(), true, &(idx[0]));
					DblNumMat dv(4, idx.size());
					DblNumVec Ylm(4); //LLIN: Spherical harmonics (0) and its derivatives (1-3)
					for(int g=0; g<idx.size(); g++) {
