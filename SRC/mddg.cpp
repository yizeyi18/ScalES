...
int main(int argc, char **argv) 
{
	...
	if( inputformat == "v1.3" ){
		...
		//scfdg
		{
			...
			scf._isOutputBases = (bool)inttmp;
			...
			//Get the block of enrichment functions
			// Enrich_Array overrides the variable enrich_number
			{
				int nenrich     = esdf_integer((char*)("Enrich_Number"), 30 );
				scf._enrichTns.resize( NElems[0], NElems[1], NElems[2] );

				int sizeEnrichArray;
				// enrich_array forms a tensor structure following the x-y-z
				// directions.
				if (esdf_block((char*)("Enrich_Array"),&sizeEnrichArray)) {
					assert( sizeEnrichArray == NElems[0] * NElems[1] * NElems[2] );
					for( int k = 0; k < NElems[2]; k++ ){
						for( int j = 0; j < NElems[1]; j++ ){
							for( int i = 0; i < NElems[0]; i++ ){
								sscanf(block_data[i+j*NElems[0]+k*NElems[0]*NElems[1]],"%d", &scf._enrichTns(i,j,k) );
							}
						}
					}
				}
