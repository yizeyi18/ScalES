...
if(1)
    ...
    Nele = N1*N2*N3;
    Neigperele = 69/64; %number of eigpars per element
    Neigttl = round(Neigperele*Nele); %number of total eigpairs wanted
end

if(1)
    Norbperele = 7; %number of orbitals per element
    Neigbuf = 3*3*3*(5);
    delta = 0.00001; %control the width of the weight wall
    bc = 'd';
    gamma = 1e-3; %combination weight;
    epsqr = 1e-4; %SVD CUTOFF VALUE
    
    fprintf(1, 'Norbperele %d\n', Norbperele);
    fprintf(1, 'Neigbuf    %d\n', Neigbuf);
    fprintf(1, 'delta      %d\n', delta);
    fprintf(1, 'gamma      %d\n', gamma);
    fprintf(1, 'epsqr      %d\n', epsqr);
...
if(1)
    ...
    for g1=1:N1
        for g2=1:N2
            for g3=1:N3
                ...
                votaux = votaux + 100*(-exp(-((gx1/s1).^2+(gx2/s2).^2+(gx3/s3).^2)));
            end
        end
    end
    vot = cell(N1,N2,N3);
    for i1=1:N1
        for i2=1:N2
            for i3=1:N3
                aux = votaux([1:Nlbl1]+(i1-1)*Nlbl1, [1:Nlbl2]+(i2-1)*Nlbl2, [1:Nlbl3]+(i3-1)*Nlbl3);
                vot{i1,i2,i3} = aux;
            end
        end
    end
...
if(1)
    ...
    tic; [A, M, INDEX] = setupDG(N1,N2,N3, L1,L2,L3, Nlbl1,Nlbl2,Nlbl3, alpha,beta, basis_all, vot); toc;
    Nbas = size(A,1);
    tic; [V, E] = eig(A);    E = diag(E); toc;
    E = sort(real(E));
    fprintf(1, 'eigvals from adaptive basis functions\n');
    eigext = E(1:Neigttl) / (beta*4*pi^2);
    
    V = V(:,1:Neigttl);
    Gram = V*V';
    rhoext = zeros(size(ox1));
    for i1=1:N1
        for i2=1:N2
            for i3=1:N3
                basis = basis_all{i1,i2,i3};
                nbasis = size(basis,1);
                tmp = zeros(Nlbl1*Nlbl2*Nlbl3,nbasis);
                for g=1:nbasis
                    tmp(:,g) = basis{g}(:);
                end
                ind = INDEX{i1,i2,i3};
                aux = diag(tmp*Gram(ind,ind)*tmp');
                aux = reshape(aux, [Nlbl1,Nlbl2,Nlbl3]);
                %rho{i1,i2,i3} = aux;
                rhoext([1:Nlbl1]+(i1-1)*Nlbl1, [1:Nlbl2]+(i2-1)*Nlbl2, [1:Nlbl3]+(i3-1)*Nlbl3) = aux;
            end
        end
    end
end
...
if(1)
    ...
    for i1=1:N1
        for i2=1:N2
            for i3=1:N3
                ...
                %get the full transformation matrix Tt = St * Ct * Ht;
                Tt = St*(Ct*Ht);
    ...
    Cold = C;
    %[u,s,v] = svd(C,'econ');    gud = find(diag(s)>1e-6*s(1,1));    C = u(:,gud);
    [q,r,e] = qr(C,0);
    gud = find(abs(diag(r))>epsqr*abs(r(1,1)));
    C = q(:,gud)*r(gud,gud);
end
...
if(1)
    ...
    Ac = (Ac+Ac')/2;
    Sc = (Sc+Sc')/2;
    tic; [Vc, Ec] = eig(Ac, Sc);    Ec = diag(Ec); toc;
    %tic; [Vc, Ec] = eigs(Ac, Sc, Neigttl+10, 'SA');    Ec = diag(Ec); toc;
    Ec = sort(real(Ec));
    fprintf(1, 'eigvals from locally contracted adaptive basis functions\n');
    eigapp = Ec(1:Neigttl) / (beta*4*pi^2);
    
    Vc = Vc(:,1:Neigttl);
    Gram = (C*Vc)*(C*Vc)';
    rhoapp = zeros(size(ox1));
    for i1=1:N1
        for i2=1:N2
            for i3=1:N3
                basis = basis_all{i1,i2,i3};
                ...
                aux = reshape(aux, [Nlbl1,Nlbl2,Nlbl3]);
                %rho{i1,i2,i3} = aux;
                rhoapp([1:Nlbl1]+(i1-1)*Nlbl1, [1:Nlbl2]+(i2-1)*Nlbl2, [1:Nlbl3]+(i3-1)*Nlbl3) = aux;
            end
        end
    end
end
...
if(1)
    ...
    tx2 = [];        for g2=1:N2            tx2 = [tx2; x2+(g2-1/2)*h2];        end
    tx3 = [];        for g3=1:N3            tx3 = [tx3; x3+(g3-1/2)*h3];        end
    figure(1); imagesc(tx1,tx2,rhoext(:,:,end/2)); colorbar; title('rho exact');
    figure(2); imagesc(tx1,tx2,rhoapp(:,:,end/2)); colorbar; title('rho approximation');
    figure(3); imagesc(tx1,tx2,votaux(:,:,end/2)); colorbar; title('potential');

    rhoerr = rhoext-rhoapp;
    norm(rhoerr(:))/norm(rhoext(:))
end
