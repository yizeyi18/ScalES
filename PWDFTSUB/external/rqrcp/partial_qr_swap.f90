! DOING PARTIAL QR ON A, WILL ALSO SWAP SWAP_A, SWAP_IPIV CORRESPONDING TO SWAPPING A AND IPIV

! AUTHOR: JIANWEI XIAO

SUBROUTINE PARTIAL_QR_SWAP( PARTIAL, M, N, A, IA, JA, DESCA, IPIV, TAU, WORK, LWORK, INFO, &
     SWAP_M, SWAP_LDA, SWAP_A, SWAP_DESCA, SWAP_IPIV )
!
!     .. Scalar Arguments ..
     INTEGER            IA, JA, INFO, LWORK, M, N, PARTIAL, SWAP_M, SWAP_LDA
!     ..
!     .. Array Arguments ..
     INTEGER            DESCA( * ), IPIV( * ), SWAP_IPIV( * ), SWAP_DESCA( * )
     DOUBLE PRECISION   A( * ), TAU( * ), WORK( * ), SWAP_A( * )
!     ..
!     .. Parameters ..
...
!     .. Local Scalars ..
     ...
     LDA, LL, LWMIN, MN, MP, MYCOL, MYROW, NPCOL, NPROW, NQ, NQ0, PVT, SWAP_IAROW, SWAP_MP
...
!
     ...
     ELSE
          ...
          IF( INFO.EQ.0 ) THEN
               ...
               !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
               SWAP_MP = NUMROC( SWAP_M, SWAP_DESCA( MB_ ), MYROW, IAROW, NPROW )
               !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
          ...
          IF( J.NE.PVT ) THEN
               ...
               IF( ICURCOL.EQ.IPCOL ) THEN
                    IF( MYCOL.EQ.ICURCOL ) THEN
                         ...
                         !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                         CALL DSWAP( SWAP_MP, SWAP_A( IIA+(JJ-1)*SWAP_LDA ), 1, SWAP_A( IIA+(JJPVT-1)*SWAP_LDA ), 1 )
                         !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                         ITEMP = IPIV( JJPVT )
                         IPIV( JJPVT ) = IPIV( JJ )
                         IPIV( JJ ) = ITEMP
                         !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                         ITEMP = SWAP_IPIV( JJPVT )
                         SWAP_IPIV( JJPVT ) = SWAP_IPIV( JJ )
                         SWAP_IPIV(JJ) = ITEMP
                         !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!
                         ...
                         !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                         CALL DGESD2D( ICTXT, SWAP_MP, 1, SWAP_A( IIA+(JJ-1)*SWAP_LDA ), SWAP_LDA, MYROW, IPCOL )
                         !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                         WORK( IPW )   = DBLE( IPIV( JJ ) )
                         WORK( IPW+1 ) = WORK( IPN + JJ - 1 )
                         WORK( IPW+2 ) = WORK( IPN + NQ + JJ - 1 )
                         !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                         WORK( IPW+3 ) = DBLE( SWAP_IPIV(JJ) )
                         !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                         !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                         CALL DGESD2D( ICTXT, 4, 1, WORK( IPW ), 4, MYROW, IPCOL )
                         !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
                         CALL DGERV2D( ICTXT, MP, 1, A( IIA+(JJ-1)*LDA ), LDA, MYROW, IPCOL )
                         !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                         CALL DGERV2D( ICTXT, SWAP_MP, 1, SWAP_A( IIA+(JJ-1)*SWAP_LDA ), SWAP_LDA, MYROW, IPCOL )
                         !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                         CALL IGERV2D( ICTXT, 1, 1, IPIV( JJ ), 1, MYROW, IPCOL )
                         !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                         CALL IGERV2D( ICTXT, 1, 1, SWAP_IPIV( JJ ), 1, MYROW, IPCOL )
                         !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
...
!
                         ...
                         !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                         CALL DGESD2D( ICTXT, SWAP_MP, 1, SWAP_A( IIA+(JJPVT-1)*SWAP_LDA ), SWAP_LDA, MYROW, ICURCOL )
                         !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                         CALL IGESD2D( ICTXT, 1, 1, IPIV( JJPVT ), 1, MYROW, ICURCOL )
                         !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                         CALL IGESD2D( ICTXT, 1, 1, SWAP_IPIV( JJPVT ), 1, MYROW, ICURCOL )
                         !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
                         CALL DGERV2D( ICTXT, MP, 1, A( IIA+(JJPVT-1)*LDA ), LDA, MYROW, ICURCOL )
                         !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                         CALL DGERV2D( ICTXT, SWAP_MP, 1, SWAP_A( IIA+(JJPVT-1)*SWAP_LDA ), SWAP_LDA, MYROW, ICURCOL )
                         !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                         !CALL DGERV2D( ICTXT, 3, 1, WORK( IPW ), 3, MYROW, ICURCOL )
                         CALL DGERV2D( ICTXT, 4, 1, WORK( IPW ), 4, MYROW, ICURCOL )
                         IPIV( JJPVT ) = IDINT( WORK( IPW ) )
                         WORK( IPN+JJPVT-1 ) = WORK( IPW+1 )
                         WORK( IPN+NQ+JJPVT-1 ) = WORK( IPW+2 )
                         !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                         SWAP_IPIV( JJPVT ) = IDINT( WORK( IPW+3 ) )
                         !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
                    END IF
!
