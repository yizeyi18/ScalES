...
// TODO Change MPI_COMM_WORLD type communication to use communicator
...
enum {
	PARVEC_REP = 0, //replace
	PARVEC_CMB = 1, //combine
};
...
public:
	...
	int putEnd(const vector<int>& mask, int putmode=0);
	//
...
int ParVec<Key,Data,Partition>::getBegin( int (*e2ps)(Key,Data&,vector<int>&), const vector<int>& mask )
{
	...
	reqs = (MPI_Request *) malloc(2*mpisize * sizeof(MPI_Request)); iA(reqs!=NULL);
	stats = (MPI_Status *) malloc(2*mpisize * sizeof(MPI_Status));
	//reqs.resize(2*mpisize);
	//stats.resize(2*mpisize);
	//---------
	vector<ostringstream*> ossvec(mpisize);  for(int k=0; k<mpisize; k++)  { ossvec[k] = new ostringstream(); iA(ossvec[k]!=NULL); }
	//1. serialize
...
int ParVec<Key,Data,Partition>::getBegin(vector<Key>& keyvec, const vector<int>& mask)
{
	...
	rbufvec.resize(mpisize);
	reqs = (MPI_Request *) malloc(2*mpisize * sizeof(MPI_Request)); iA(reqs!=NULL);
	stats = (MPI_Status *) malloc(2*mpisize * sizeof(MPI_Status)); iA(stats!=NULL);
	//reqs.resize(2*mpisize);
	//stats.resize(2*mpisize);
	...
	t1 = time(0);
	//  if(mpirank == 0 ){
	//    cerr << "serialize time cost " << difftime(t1,t0) << endl;
	//  }
...
int ParVec<Key,Data,Partition>::getEnd( const vector<int>& mask )
{
	...
	//4. write back
	//to stream
	vector<istringstream*> issvec(mpisize);  for(int k=0; k<mpisize; k++)	{ issvec[k] = new istringstream(); iA(issvec[k]!=NULL); }
	for(int k=0; k<mpisize; k++) {
	...
	t1 = time(0);
	//  if(mpirank == 0 ){
	//    cerr << "deserialize time cost " << difftime(t1,t0) << endl;
	//  }
...
//--------------------------------------------
template <class Key, class Data, class Partition>
int ParVec<Key,Data,Partition>::putEnd( const vector<int>& mask, int putmode )
{
	int mpirank = this->mpirank();
	int mpisize = this->mpisize();
	//LEXING: SEPARATE HERE
	...
	for(int k=0; k<mpisize; k++) {
		for(int i=0; i<rnbvec[k]; i++) {
			...
			/*
				 typename map<Key,Data>::iterator mi=_lclmap.find(key);	  iA( mi!=_lclmap.end() ); //VERY IMPORTANT, MAKE SURE MY OWN COPY EXISTS
				 if(       putmode==PARVEC_REP) {
				 deserialize((*mi).second, *(issvec[k]), mask);
				 } else if(putmode==PARVEC_CMB) {
				 Data tmp;	deserialize(tmp, *(issvec[k]), mask);
				 combine( (*mi).second, tmp); //LEXING: COMBINE
				 }
				 */
			typename map<Key,Data>::iterator mi=_lclmap.find(key);
			if(mi==_lclmap.end() ) { //DO NOT EVEN EXIST
				//----------------------------------
				Data tmp;	deserialize(tmp, *(issvec[k]), mask);
				_lclmap[key] = tmp;
			} else { //DO EXIST
...
// *********************************************************************
// Commonly used ParVecs
// *********************************************************************

// Block row partition that is consistent with the parallel FFT
// partition.
struct BlockRowPtn
{
	IntNumVec  ownerInfo;
	// OWNER is determined by the row index only
	bool owner(const Int key) {
		return ownerInfo(key);
	}
};

#define ParNumVec<F>        ParVec<Int, NumVec<F>, BlockRowPtn>
#define ParNumMat<F>        ParVec<Int, NumMat<F>, BlockRowPtn>
